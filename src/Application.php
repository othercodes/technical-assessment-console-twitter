...
namespace Lookiero\Hiring\ConsoleTwitter;
...
class Application
{
    ...
    /**
     * Server API (cli, http...), the interface between the server and
     * the php runtime.
     * @var string
     */
    protected $sapi;
    ...
    /**
     * Bind the given kernel to a certain sapi type.
     * @param string $sapi
     * @param string $kernel
     * @return Application
     */
    public function kernel(string $sapi, $kernel): Application
    {
        $this->container->set("kernel.{$sapi}", $kernel instanceof KernelContract
            ? $kernel : function (ContainerContract $container) use ($kernel) {
    ...
    public function run(InputContract $input, OutputContract $output): int
    {
         ...
         */
        $kernel = $this->container->get("kernel.{$this->sapi}");
